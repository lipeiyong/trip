package net.lvtushiguang.trip.db;

import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;

import net.lvtushiguang.trip.bean.NotebookData;
...
public class NoteDatabase {
    ...
    public void delete(int id) {
        SQLiteDatabase sqlite = dbHelper.getWritableDatabase();
        String sql = ("list_cell_delete from " + DatabaseHelper.NOTE_TABLE_NAME + " where _id=?");
    ...
    public List<NotebookData> query(String where) {
        ...
        for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
            ...
            notebookData.setId(cursor.getString(0));
    ...
    public void reset(List<NotebookData> datas) {
        if (datas != null) {
            SQLiteDatabase sqlite = dbHelper.getWritableDatabase();
            // 删除全部
            sqlite.execSQL("list_cell_delete from " + DatabaseHelper.NOTE_TABLE_NAME);
            // 重新添加
